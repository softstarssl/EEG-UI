...
def create_inference_service(
    ...
    if inference_settings.backend == "http_reve":
        if server_settings is None:
            raise ValueError("Server settings are required for the http_reve backend.")
        return HttpReveInferenceService(
            server=server_settings,
            threshold=inference_settings.threshold,
        )
    raise ValueError(
        f"Unknown inference backend '{inference_settings.backend}'. "
        "Use 'mock_reve', 'real_reve', or 'http_reve'."
    )
